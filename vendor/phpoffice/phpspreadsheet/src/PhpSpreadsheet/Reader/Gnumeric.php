...
class Gnumeric extends BaseReader
{
    ...
    public function loadIntoExisting($pFilename, Spreadsheet $spreadsheet)
    {
        ...
        $namespacesExtra = $xml->getNamespaces(true);

        $gnmXML = $xml->children($namespacesExtra['gnm']);

        $docProps = $spreadsheet->getProperties();
        //    Document Properties are held differently, depending on the version of Gnumeric
        if (isset($namespacesExtra['office'])) {
            $officeXML = $xml->children($namespacesExtra['office']);
            $officeDocXML = $officeXML->{'document-meta'};
            $officeDocExtraXML = $officeDocXML->meta;

            foreach ($officeDocExtraXML as $officePropertyData) {
                $officePropertyDC = [];
                if (isset($namespacesExtra['dc'])) {
                    $officePropertyDC = $officePropertyData->children($namespacesExtra['dc']);
                }
                ...
                $officePropertyExtra = [];
                if (isset($namespacesExtra['extra'])) {
                    $officePropertyExtra = $officePropertyData->children($namespacesExtra['extra']);
                }
                foreach ($officePropertyExtra as $propertyName => $propertyValue) {
                    $attributes = $propertyValue->attributes($namespacesExtra['extra']);
                    $propertyValue = (string) $propertyValue;
                    switch ($propertyName) {
                        case 'keyword':
