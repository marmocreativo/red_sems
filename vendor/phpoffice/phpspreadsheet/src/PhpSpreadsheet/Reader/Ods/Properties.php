...
class Properties
{
    ...
    public function load(\SimpleXMLElement $xml, $namespacesExtra)
    {
        $docProps = $this->spreadsheet->getProperties();
        $officeProperty = $xml->children($namespacesExtra['office']);
        foreach ($officeProperty as $officePropertyData) {
            /** @var \SimpleXMLElement $officePropertyData */
            $officePropertiesDC = (object) [];
            if (isset($namespacesExtra['dc'])) {
                $officePropertiesDC = $officePropertyData->children($namespacesExtra['dc']);
            }
            $this->setCoreProperties($docProps, $officePropertiesDC);

            $officePropertyExtra = (object) [];
            if (isset($namespacesExtra['dc'])) {
            ...
            foreach ($officePropertyExtra as $propertyName => $propertyValue) {
                $this->setExtraProperties($namespacesExtra, $propertyValue, $propertyName, $docProps);
            }
        }
    }
    ...
    private function setExtraProperties(
        $namespacesExtra,
        \SimpleXMLElement $propertyValue,
        $propertyName,
        DocumentProperties $docProps
    ) {
        $propertyValueAttributes = $propertyValue->attributes($namespacesExtra['extra']);
        $propertyValue = (string) $propertyValue;
        switch ($propertyName) {
