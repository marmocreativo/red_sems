...
class Chart
{
    ...
    {
        $namespacesChartExtra = $chartElements->getNamespaces(true);
        $chartElementsC = $chartElements->children($namespacesChartExtra['c']);
            ...
            switch ($chartElementKey) {
                case 'chart':
                    foreach ($chartElement as $chartDetailsKey => $chartDetails) {
                        $chartDetailsC = $chartDetails->children($namespacesChartExtra['c']);
                        switch ($chartDetailsKey) {
                            case 'plotArea':
                                ...
                                foreach ($chartDetails as $chartDetailKey => $chartDetail) {
                                    switch ($chartDetailKey) {
                                        case 'layout':
                                            $plotAreaLayout = self::chartLayoutDetails($chartDetail, $namespacesChartExtra);

                                            break;
                                        case 'catAx':
                                            if (isset($chartDetail->title)) {
                                                $XaxisLabel = self::chartTitle($chartDetail->title->children($namespacesChartExtra['c']), $namespacesChartExtra);
                                            }
                                        ...
                                        case 'valAx':
                                            if (isset($chartDetail->title)) {
                                                $YaxisLabel = self::chartTitle($chartDetail->title->children($namespacesChartExtra['c']), $namespacesChartExtra);
                                            }
                                        ...
                                        case 'bar3DChart':
                                            ...
                                            $plotSer = self::chartDataSeries($chartDetail, $namespacesChartExtra, $chartDetailKey);
                                        ...
                                        case 'lineChart':
                                        case 'line3DChart':
                                            $plotSeries[] = self::chartDataSeries($chartDetail, $namespacesChartExtra, $chartDetailKey);
                                            $plotAttributes = self::readChartAttributes($chartDetail);
                            ...
                            case 'dispBlanksAs':
                                ...
                                break;
                            case 'title':
                                $title = self::chartTitle($chartDetails, $namespacesChartExtra);
                                ...
                                foreach ($chartDetails as $chartDetailKey => $chartDetail) {
                                    switch ($chartDetailKey) {
                                        ...
                                        case 'overlay':
                                            ...
                                            break;
                                        case 'layout':
                                            $legendLayout = self::chartLayoutDetails($chartDetail, $namespacesChartExtra);
    ...
    private static function chartTitle(SimpleXMLElement $titleDetails, array $namespacesChartExtra)
    {
        $caption = [];
        $titleLayout = null;
        foreach ($titleDetails as $titleDetailKey => $chartDetail) {
            switch ($titleDetailKey) {
                case 'tx':
                    $titleDetails = $chartDetail->rich->children($namespacesChartExtra['a']);
                    foreach ($titleDetails as $titleKey => $titleDetail) {
                        switch ($titleKey) {
                            case 'p':
                                $titleDetailPart = $titleDetail->children($namespacesChartExtra['a']);
                    ...
                    break;
                case 'layout':
                    $titleLayout = self::chartLayoutDetails($chartDetail, $namespacesChartExtra);
    ...
    private static function chartLayoutDetails($chartDetail, $namespacesChartExtra)
    {
        if (!isset($chartDetail->manualLayout)) {
            return null;
        }
        $details = $chartDetail->manualLayout->children($namespacesChartExtra['c']);
        if ($details === null) {
            return null;
        }
        $layout = [];
    ...
    private static function chartDataSeries($chartDetail, $namespacesChartExtra, $plotType)
    {
        $multiSeriesType = null;
        $smoothLine = false;
        $seriesLabel = $seriesCategory = $seriesValues = $plotOrder = [];

        $seriesDetailSet = $chartDetail->children($namespacesChartExtra['c']);
                    ...
                    foreach ($seriesDetails as $seriesKey => $seriesDetail) {
                        switch ($seriesKey) {
                            ...
                            case 'order':
                                ...
                                break;
                            case 'tx':
                                $seriesLabel[$seriesIndex] = self::chartDataSeriesValueSet($seriesDetail, $namespacesChartExtra);
                            ...
                            case 'smooth':
                                ...
                                break;
                            case 'cat':
                                $seriesCategory[$seriesIndex] = self::chartDataSeriesValueSet($seriesDetail, $namespacesChartExtra);

                                break;
                            case 'val':
                                $seriesValues[$seriesIndex] = self::chartDataSeriesValueSet($seriesDetail, $namespacesChartExtra, $marker);

                                break;
                            case 'xVal':
                                $seriesCategory[$seriesIndex] = self::chartDataSeriesValueSet($seriesDetail, $namespacesChartExtra, $marker);
    ...
    private static function chartDataSeriesValueSet($seriesDetail, $namespacesChartExtra, $marker = null)
    {
        if (isset($seriesDetail->strRef)) {
            $seriesSource = (string) $seriesDetail->strRef->f;
            $seriesData = self::chartDataSeriesValues($seriesDetail->strRef->strCache->children($namespacesChartExtra['c']), 's');
            ...
            $seriesData = self::chartDataSeriesValues($seriesDetail->numRef->numCache->children($namespacesChartExtra['c']));
            ...
            $seriesData = self::chartDataSeriesValuesMultiLevel($seriesDetail->multiLvlStrRef->multiLvlStrCache->children($namespacesChartExtra['c']), 's');
            $seriesData['pointCount'] = count($seriesData['dataValues']);

            return new DataSeriesValues(DataSeriesValues::DATASERIES_TYPE_STRING, $seriesSource, $seriesData['formatCode'], $seriesData['pointCount'], $seriesData['dataValues'], $marker);
        } elseif (isset($seriesDetail->multiLvlNumRef)) {
            $seriesSource = (string) $seriesDetail->multiLvlNumRef->f;
            $seriesData = self::chartDataSeriesValuesMultiLevel($seriesDetail->multiLvlNumRef->multiLvlNumCache->children($namespacesChartExtra['c']), 's');
            $seriesData['pointCount'] = count($seriesData['dataValues']);
