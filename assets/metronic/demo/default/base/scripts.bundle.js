				...
				var afterGetData = function(result) {
					...
					if (options.pagination) {
						if (options.data.serverPaging && options.data.type !== 'local') {
							// server pagination
							var serverExtra = Plugin.getObject('extra', result || null);
							if (serverExtra !== null) {
								Plugin.pagingObject = Plugin.paging(serverExtra);
							} else {
								// no meta object from server response, fallback to local pagination
								Plugin.pagingObject = Plugin.paging(buildExtra(), localPagingCallback);
							}
						} else {
